...
def run_RT_for_emul(Mhalo,fstar,parameters,Helium):
    ...
    pickle.dump(file=open('./profiles_output/dict_profiles_fst_' + str(round(fstar,2)) + '_zi{}_Mh_{:.1e}.pkl'.format(z_start, Mhalo), 'wb'),
                obj={'z':grid_model.z_history,'r':grid_model.r_grid_cell,'Temp':grid_model.T_history,'xHII':grid_model.xHII_history} )
...
def paint_ion_profile_emulator(filename,param,fstar):
    ...
    profile = pickle.load(file=open('./profiles_output/dict_profiles_fst_0.1_zi25_Mh_1.0e+05.pkl','rb'))

                                    #'dict_profiles_fst_' + str(round(Sampling[0][1], 2)) + '_zi{}_Mh_{:.1e}.pkl'.format(z_start, Mhalo),'rb'))
