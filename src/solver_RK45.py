...
from .couplings import x_coll, rho_alpha, S_alpha, J0_xray_lyal, sigma_HI
import copy
    ...
    def solve(self, param):
        ...
        while True:
            ...
            while zstep_l > param.solver.z_end :
                ...
                for k in (arange(0, self.r_grid_cell.size, 1)):

                    def rhs(t, n):
                        ## n[0] is nHII and n[1] is Tx*nB

                        n_ee =  n[0] #n_HII_cell  #e- density
                        n_HI___= nB_profile_z[k]- n[0]
                        mu = nB_profile_z[k]/ (nB_profile_z[k] +  n[0])  #molecular weigth
                        Tx_Nb = n[1]
                        Tx = Tx_Nb / nB_profile_z[k]
                        ##coeff for Temp eq
                        A1_HI = xi_HI(Tx) * n_HI___ * n_ee
                        A2_HII = eta_HII(Tx) * n[0]  * n_ee
                        A4_HI = psi_HI(Tx) * n_HI___ * n_ee
                        A5 = theta_ff(Tx) * (n[0]) * n_ee
                        H = pl.H(zstep_l)
                        H = H.to(u.s ** -1).value
                        A6 = (15 / 2 * H * kb_eV_per_K * Tx_Nb / mu)

                        A = gamma_HI(n[0], n_HI___, Tx, I1_HI[k], I2_HI[k], gamma_2c) * n_HI___ - alpha_HII(Tx) * n[0] * n_ee
                        D = (2 / 3) * mu / (kb_eV_per_K) * (f_Heat(n[0] / nB_profile_z[k]) * (n_HI___ * I1_T_HI[k]) + sigma_s * n_ee / m_e_eV * (I2_Ta[k] + Tx * I2_Tb[k]) - (A1_HI + A2_HII + A4_HI + A5 + A6))

                        return ravel(array([A, D], dtype="object"))

                    y0 = zeros(2)
                    y0[0] = n_HII_cell[k]
                    y0[1] = T_grid[k] * nB_profile_z[k]
                    if param.solver.method == 'sol':
                        sol = integrate.solve_ivp(rhs, [l * dt_init.value, (l + 1) * dt_init.value], y0, method='RK45')
                        n_HII_cell[k]= sol.y[0, -1]
                        T_grid[k] = nan_to_num(sol.y[1, -1]) / (nB_profile_z[k] * (1 + zstep_l) ** 3 / (1 + z_previous) ** 3)
